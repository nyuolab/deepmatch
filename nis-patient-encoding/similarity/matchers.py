...
class Matcher(object):
    def __init__(self, data_fn, filtered_inds):
        """Initialize with necessary components."""

        self.db = NISDatabase(
            data_fn, 
            'case',
            state_inds=filtered_inds)

        # self.case_db = NISDatabase(data_fn, 'TRAIN', {})
        # self.control_db = NISDatabase(data_fn, 'TRAIN', {})
    ...
    def prepare_for_matching(self):
        ...
        # self.case_db.set_inds(self.cb.cases)
        # self.control_db.set_inds(self.cb.controls)

        # self.all_case_dl = torch.utils.data.DataLoader(self.case_db, batch_size=1000, pin_memory=True, num_workers=4)
        # self.all_control_dl = torch.utils.data.DataLoader(self.control_db, batch_size=1000, pin_memory=True, num_workers=4)

    def match(self):
        raise NotImplementedError

class DeepMatcher(Matcher):
    def __init__(self, data_fn, filtered_inds, model, device='cpu', save_dir=None):
        """Initialize with necessary components.
        
        Model must contain loaded weights.
        
        """
        super().__init__(data_fn, filtered_inds)

        self.data_fn = data_fn
        self.filename = self.data_fn.split('/')[-1].split('.h5')[0]
        self.model = model
        self.DEVICE = device
